...
def ojos_abiertos(landmarks, image_shape):
    ...
    EAR_CERRADO = 0.12      # Solo si está prácticamente cerrado
    EAR_MAX = 0.34          # Evita ojos exageradamente abiertos
    DIF_MAX = 0.22          # Permite asimetría natural

    diferencia = abs(ear_izq - ear_der)
    ear_prom = (ear_izq + ear_der) / 2

    # 1️⃣ No aceptar si ambos están cerrados
    if ear_izq < EAR_CERRADO and ear_der < EAR_CERRADO:
        return False

    # 2️⃣ No aceptar si están exageradamente abiertos
    if ear_prom > EAR_MAX:
        return False

    # 3️⃣ No aceptar si la diferencia es extrema (guiño forzado)
...
def expresion_neutra(landmarks, image_shape):
    ...
    #no_sonrisa = sonrisa < 0.48  # Ajustable
     ...
     # ========= UMBRALES =========
    # MAR mínimo para mostrar dientes
    mar_min = 0.35

    # MAR máximo para evitar sorpresa exagerada
    mar_max = 0.65

    # Ensanchamiento horizontal típico de sonrisa
    sonrisa_min = 0.50

    if mar_min < mar < mar_max and sonrisa > sonrisa_min:
        return True
...
def es_frontal(landmarks, tolerancia_ojos_y = 0.04, tolerancia_nariz_x = 0.05, tolerancia_angulo=7):
    ...
    #angle = np.arctan2(ojo_der[1] - ojo_izq[1], ojo_der[0] - ojo_izq[0])
...
def detectar_rostro_pdf(pdf_bytes):
    ...
    for pagina in pdf_documents:
        ...
        for img in imagenes:
            ...
            rostros, razon = detectar_rostro_imagen(imagen_bytes)

            
            # Solo aceptar la primera imagen válida
            if rostros == 1 and total_rostros == 0:
                total_rostros += 1
