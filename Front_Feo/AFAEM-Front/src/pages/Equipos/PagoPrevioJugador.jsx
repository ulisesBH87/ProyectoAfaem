...
export default function PagoPrevioJugador() {
  ...
  const renderComprobanteBox = ({ helperText, buttonText }) => (
    <>
      ...
      <button
        ...
        style={{ width: '100%', marginTop: '24px', padding: '14px 18px', borderRadius: '12px', border: 'none', background: uploadingComprobante ? COLORS.slate400 : COLORS.primary, color: 'white', fontWeight: '900', cursor: uploadingComprobante ? 'wait' : (!comprobanteFile ? 'not-allowed' : 'pointer'), opacity: comprobanteFile ? 1 : 0.55 }}
      >
  ...
  const renderResumenOrden = () => (
    <div className="pago-card">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 0', borderBottom: `1px solid ${COLORS.slate100}`, color: COLORS.slate500, fontSize: '13px' }}>
        <span>Orden de pago</span>
        <strong style={{ color: COLORS.slate800 }}>#{resolvedOrdenId || '-'}</strong>
      </div>
      {detallesSeguros.map((detalle, index) => {
        const seguro = segurosCatalogo.find(item => Number(item.id) === Number(detalle.SeguroId));
        return (
          <div key={`${detalle.SeguroId}-${index}`} style={{ display: 'flex', justifyContent: 'space-between', padding: '9px 0', borderBottom: `1px solid ${COLORS.slate100}`, color: COLORS.slate500, fontSize: '13px' }}>
            <span>{seguro?.nombre || `Seguro ${detalle.SeguroId}`} x{Number(detalle.Cantidad || 0)}</span>
            <strong style={{ color: COLORS.slate800 }}>${Number(detalle.Subtotal || 0).toFixed(2)}</strong>
          </div>
        );
      })}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '18px', paddingTop: '18px', borderTop: `2px solid ${COLORS.slate200}` }}>
        ...
        <span style={{ fontSize: '24px', fontWeight: '900', color: COLORS.primary }}>${totalResumen.toFixed(2)}</span>
      </div>

      <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: `2px solid ${COLORS.slate200}` }}>
        <h4 style={{ fontSize: '13px', fontWeight: '800', color: COLORS.primary, marginBottom: '12px', textTransform: 'uppercase' }}>Datos de Transferencia</h4>
        <div style={{ display: 'grid', gap: '8px', fontSize: '12px', color: COLORS.slate500 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Banco:</span> <strong style={{ color: COLORS.slate800 }}>{DEFAULT_BANK_INFO.banco}</strong></div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Titular:</span> <strong style={{ color: COLORS.slate800, textAlign: 'right', maxWidth: '65%' }}>{DEFAULT_BANK_INFO.titular}</strong></div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Cuenta:</span> <strong style={{ color: COLORS.slate800 }}>{DEFAULT_BANK_INFO.cuenta}</strong></div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>CLABE:</span> <strong style={{ color: COLORS.slate800 }}>{DEFAULT_BANK_INFO.clabe}</strong></div>
        </div>
      </div>
    </div>
  );
    ...
    return (
      <div style={{ maxWidth: '980px', margin: '0 auto', animation: 'slideUp 0.4s ease' }}>
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          ...
          <h2 style={{ fontSize: '28px', fontWeight: '900', color: COLORS.slate800, marginBottom: '8px' }}>Pago previo para agregar jugadores</h2>
          ...
          <p style={{ color: COLORS.dangerDark, margin: 0 }}>
        ...
        <div className="responsive-pago-grid">
          <div className="pago-card">
            {!ordenCreada ? (
              <>
                    ...
                    return (
                      ...
                      >
                        ...
                        <input
                          ...
                          style={{ width: '100%', padding: '10px', border: `1px solid ${COLORS.slate300}`, borderRadius: '10px', fontWeight: '800', textAlign: 'center' }}
                        />
                      </div>
                    );
                  })}
                ...
                <div style={{ marginTop: '16px', padding: '12px 14px', borderRadius: '12px', background: totalAsignadosPagoJugador === segurosRequeridosPagoJugador && segurosRequeridosPagoJugador > 0 ? COLORS.successBg : COLORS.orange50, color: totalAsignadosPagoJugador === segurosRequeridosPagoJugador && segurosRequeridosPagoJugador > 0 ? COLORS.successDarker : COLORS.orangeDarker, fontWeight: '800', fontSize: '13px' }}>
          ...
          <div className="pago-card">
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '18px', paddingTop: '18px', borderTop: `2px solid ${COLORS.slate200}` }}>
              <span style={{ fontWeight: '900', color: COLORS.slate800 }}>Total</span>
              <span style={{ fontSize: '24px', fontWeight: '900', color: COLORS.primary }}>${Number(totalPagoMostradoJugador || 0).toFixed(2)}</span>
            </div>
      ...
      <div className="fade-in" style={{ maxWidth: '800px', margin: '0 auto' }}>
        ...
        <div className="card" style={{ padding: '40px', borderRadius: '24px', border: 'none' }}>
          ...
          <div style={{
            ...
            <p style={{ color: COLORS.secondaryHover, fontWeight: '600', marginBottom: '12px' }}>
              📋 Próximos pasos:
            </p>
            <ol style={{ marginLeft: '20px', color: COLORS.secondaryHover, lineHeight: '1.8' }}>
          ...
          <div className="card" style={{ padding: '40px', borderRadius: '24px', border: 'none' }}>
            <div style={{ textAlign: 'center', marginBottom: '32px' }}>
              <div style={{
                ...
                <FaFileUpload style={{ color: COLORS.warningBrown }} />
              </div>
              <h1 style={{ fontSize: '28px', fontWeight: '800', color: 'var(--text-main)', marginBottom: '12px' }}>
                Pago Previo para Nuevo Jugador(es)
              </h1>
            ...
            <div style={{
              ...
              <p style={{ color: COLORS.orangeDeep, fontWeight: '600', marginBottom: '8px' }}>
                ⚠️ Información importante:
              </p>
              <ul style={{ marginLeft: '20px', color: COLORS.orangeDeep, lineHeight: '1.8', marginBottom: 0 }}>
        ...
        <div className="card" style={{ padding: '40px', borderRadius: '24px', border: 'none' }}>
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            <div style={{
              ...
              <FaClock style={{ color: COLORS.dangerLight }} />
            </div>
            <h1 style={{ fontSize: '28px', fontWeight: '800', color: 'var(--text-main)', marginBottom: '12px' }}>
              Pago Previo para Nuevo Jugador(es)
            </h1>
          ...
          <div style={{
            ...
            <p style={{ color: COLORS.dangerDeep, lineHeight: '1.6', marginBottom: 0 }}>
        ...
        <div className="card" style={{ padding: '40px', borderRadius: '24px', border: 'none' }}>
          <div style={{ textAlign: 'center', marginBottom: '32px' }}>
            <div style={{
              ...
              <FaFileUpload style={{ color: COLORS.dangerDark }} />
            </div>
            <h1 style={{ fontSize: '28px', fontWeight: '800', color: 'var(--text-main)', marginBottom: '12px' }}>
              Pago Previo para Nuevo Jugador(es)
            </h1>
          ...
          <div style={{
            ...
            <p style={{ color: COLORS.dangerDeep, fontWeight: '600', marginBottom: '12px' }}>
